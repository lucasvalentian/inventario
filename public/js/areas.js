...
function abrimodal(id){
    ...
    if(id == "0"){
        ...
        $.get(urlgeeneral+"/matenimineto-areas/editar/" + id, function (data) {
            //console.log(data["Codigo"]);
            $("#valor").val(id);
            document.getElementById("area").value = data["name"];
...
$("#guardar").on("click",function(){
    ...
    if (datosobligatorio() == true) {
      ...
      $.ajax({
        type: "POST",
        url: urlgeeneral+"/matenimineto-areas/crear",
        data: frm,
        dataType: 'json',
        contentType: false,
        processData: false,
...
$("#actualizar").on("click",function(){
    ...
    if (datosobligatorio() == true) {
              ...
              $.ajax({
                type: "POST",
                url: urlgeeneral+"/matenimineto-areas/editar",
          ...
          if (e.target.classList.contains('eliminar') || e.target.classList.contains('bx')) {
              Swal.fire({
                  ...
                  var csrf = document.querySelector('meta[name="csrf-token"]').content;
                    $.ajax({
                      type: "POST",
                      url: urlgeeneral+"/matenimineto-areas/eliminar/"+id,
